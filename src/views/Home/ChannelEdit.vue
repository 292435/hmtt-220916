...
export default {
  ...
  methods: {
    goChannelFn(ind, item) {
      if (this.isEdit === false) {
        this.$emit('goChannelEv', ind)
      } else if (item.id !== 0) {
        this.$emit('delChannelEv', item)
      }
    },
    delpannelFn() {
      this.$emit('delpannelEv')
      this.isEdit = false
    },
    addChannelFn(obj) {
      if (this.isEdit === true) {
